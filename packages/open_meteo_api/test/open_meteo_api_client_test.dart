  ...
  group('OpenMeteoApiClient', () {
    ...
    group('locationSearch', () {
      ...
      test('makes correct http request', () async {
        ...
        verify(
          () => httpClient.get(
            Uri.https('geocoding-api.open-meteo.com', '/v1/search', {'name': query, 'count': '1'}),
          ),
        ).called(1);
      });
      ...
      test('throws LocationRequestFailure on non-200 response', () async {
        ...
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        expect(() async => apiClient.locationSearch(query), throwsA(isA<LocationRequestFailure>()));
      });
      ...
      test('throws LocationNotFoundFailure on error response', () async {
        ...
        when(() => response.body).thenReturn('{}');
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        await expectLater(apiClient.locationSearch(query), throwsA(isA<LocationNotFoundFailure>()));
      });
